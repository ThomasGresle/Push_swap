...
int	use_sa_link(t_list *list_a)
{
	...
	if (list_a->size >= 2)
	{
		...
		keep_a = number_keep_a_link(list_a, list_a->begin_sort);
		move_s(list_a, NULL, NULL);
		number_keep_a_link(list_a, list_a->begin_sort);
...
void	sort_list_a_by_link(t_list *list_a, t_list *list_b,
			t_move_list *move_list)
{
	while (is_move_to_b(list_a))
	{
		if (use_sa_link(list_a))
		{
			move_s(list_a, "sa", move_list);
			list_a->keep_a = number_keep_a_link(list_a, list_a->begin_sort);
		}
...
t_move_list	*sort_by_link(t_list *list_a)
{
	...
	sort_list_a_by_link(list_a, list_b, move_list);
